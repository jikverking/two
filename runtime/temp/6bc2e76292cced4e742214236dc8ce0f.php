<?php if (!defined('THINK_PATH')) exit(); /*a:3:{s:67:"C:\wamp64\www\public/../application/admin\view\config\typelist.html";i:1654605949;s:53:"C:\wamp64\www\application\admin\view\common\head.html";i:1654616318;s:53:"C:\wamp64\www\application\admin\view\common\foot.html";i:1558175186;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
...
<script type="text/html" id="topBtn">
    <button tipsmsg="添加" class="layui-btn layui-btn layui-btn-blue layui-btn-sm onmousemovebtns"
            onclick="x_admin_show('添加应用类型','<?php echo url('typeadd'); ?>','400px','250px','95%','250px')">
        <i class="layui-icon"></i>添加
    </button>
</script>
<script type="text/html" id="action" >
    <a tipsmsg="编辑" class="layuimsg" onclick="x_admin_show('编辑应用类型','<?php echo url('typeedit'); ?>?id={{d.id}}','400px','200px','95%','200px')">
...
<script>
    layui.use(['table','form','element','laydate'], function() {
        ...
        table.on('edit(list)', function(obj){
            ...
            $.post('<?php echo url("typeedit"); ?>',arrs,function(res){
                layer.close(loading);
                if(res.code===1){
                    layer.msg(res.msg);
                    tableIn.reload();
        ...
        form.on('switch(statu)', function(obj){
            var urls = "<?php echo url('typestatu'); ?>";
            loading =layer.load(1, {shade: [0.1,'#fff']});
            var id = this.value;
        ...
        $('#delAll').click(function(){
            ...
            layer.confirm('确认要删除选中信息吗？', {icon: 3,skin: 'layui-layer-lan',anim: 4,title:'删除'}, function(index) {
                layer.close(index);
                var loading = layer.load(1, {shade: [0.1, '#fff']});
                $.post("<?php echo url('typedel'); ?>", {id: id}, function (data) {
                    layer.close(loading);
                    if (data.code === 1) {
                        layer.msg(data.msg);
                        tableIn.reload();
