<?php if (!defined('THINK_PATH')) exit(); /*a:3:{s:69:"C:\wamp64\www\public/../application/admin\view\trill\commentlist.html";i:1558175186;s:53:"C:\wamp64\www\application\admin\view\common\head.html";i:1654616318;s:53:"C:\wamp64\www\application\admin\view\common\foot.html";i:1558175186;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
    ...
    <div class="layui-card">
        <div class="layui-card-body">
            <div class="demoTable">
                ...
                <?php if(input('get.id')): ?>
                <input type="hidden" value="<?php echo input('get.id'); ?>" id="groupid">
                <button class="layui-btn layui-btns layui-btn-green" id="group">
                    <i class="fa fa-object-group" aria-hidden="true"></i>
                    分组
                </button>
                <?php else: if(!(input('get.ids'))): ?>
...
<input type="hidden" value="<?php echo input('get.ids'); ?>" id="ids">
        </div> 
    </div>  
    <br>
    <br>  
...
<!--<script type="text/html" id="topBtn">
    <button tipsmsg="添加" class="layui-btn layui-btn layui-btn-blue layui-btn-sm onmousemovebtns"
            onclick="x_admin_show('添加评论','<?php echo url('commentadd'); ?>','700px','650px','95%','95%')">
        <i class="layui-icon"></i>添加
    </button>
</script>-->
<script type="text/html" id="topBtn">
    <button tipsmsg="添加" class="layui-btn layui-btn layui-btn-blue layui-btn-sm onmousemovebtns"
            onclick="x_admin_show('添加评论','<?php echo url('commentadd'); ?>','600px','300px','95%','55%')">
        <i class="layui-icon"></i>添加
    </button>
</script>
<script type="text/html" id="action" >
    ...
    <a tipsmsg="编辑" class="layuimsg"
       onclick="x_admin_show('编辑评论信息','<?php echo url('commentedit'); ?>?id={{d.id}}','600px','300px','95%','55%')">
...
<script>
    ...
    layui.use(['table','form','element','laydate'], function() {
        ...
        $('#group').click(function(){
            ...
            cid = id.toString();
            var id = $("#groupid").val();
            $.post("<?php echo url('commentinggroup'); ?>",{cid:cid,id:id},function(res){
                if(res.code == 1){
                    layer.msg(res.msg);
                    setTimeout(function(){
        ...
        form.on('submit(commentinggroup)', function(data){
            ...
            $.post("<?php echo url('commentinggroup'); ?>",data.field,function(res){
                if(res.code == 1){
                    layer.msg(res.msg);
                    setTimeout(function(){
                        window.parent.location.reload();//修改成功后刷新父界面
        ...
        table.on('tool(list)', function(obj) {
            ...
            if (obj.event === 'del') {
                layer.confirm('您确定要删除该信息吗？', {icon: 3,skin: 'layui-layer-lan',anim: 4,title:'删除'}, function(index){
                    ...
                    $.post("<?php echo url('commentdel'); ?>",{id:data.id},function(res){
                        layer.close(loading);
                        if(res.code===1){
                            layer.msg(res.msg);
                            tableIn.reload();
        ...
        $('#delAll').click(function(){
            ...
            layer.confirm('确认要删除选中信息吗？', {icon: 3,skin: 'layui-layer-lan',anim: 4,title:'删除'}, function(index) {
                layer.close(index);
                var loading = layer.load(1, {shade: [0.1, '#fff']});
                $.post("<?php echo url('commentdel'); ?>", {id: id}, function (data) {
                    layer.close(loading);
                    if (data.code === 1) {
                        layer.msg(data.msg);
                        tableIn.reload();
