<?php if (!defined('THINK_PATH')) exit(); /*a:3:{s:66:"C:\wamp64\www\public/../application/admin\view\config\taglist.html";i:1654606094;s:53:"C:\wamp64\www\application\admin\view\common\head.html";i:1654616318;s:53:"C:\wamp64\www\application\admin\view\common\foot.html";i:1558175186;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
...
<script type="text/html" id="topBtn">
    <button tipsmsg="添加" class="layui-btn layui-btn layui-btn-blue layui-btn-sm onmousemovebtns"
            onclick="x_admin_show('添加应用标识','<?php echo url('tagadd'); ?>','400px','400px','95%','95%')">
        <i class="layui-icon"></i>添加
    </button>
</script>
<script type="text/html" id="action" >
    <a tipsmsg="编辑" class="layuimsg" onclick="x_admin_show('编辑应用标识信息','<?php echo url('tagedit'); ?>?id={{d.id}}','400px','350px','95%','95%')">
    ...
    layui.use(['table','form','element','laydate'], function() {
        ...
        var tableIn = table.render({
            ...
            cols: [[
                ...
                {field: 'name', title: '应用名称',width:200,edit: 'text'},
                {field: 'tagname', title: '应用标识',width:180,edit: 'text'},
        ...
        form.on('switch(statu)', function(obj){
            var urls = "<?php echo url('tagstatu'); ?>";
            loading =layer.load(1, {shade: [0.1,'#fff']});
            var id = this.value;
        ...
        table.on('tool(list)', function(obj) {
            ...
            if (obj.event === 'del') {
                layer.confirm('您确定要删除该信息吗？', {icon: 3,skin: 'layui-layer-lan',anim: 4,title:'删除'}, function(index){
                    ...
                    $.post("<?php echo url('tagdel'); ?>",{id:data.id},function(res){
                        layer.close(loading);
                        if(res.code===1){
                            layer.msg(res.msg);
                            tableIn.reload();
        ...
        $('#delAll').click(function(){
            ...
            layer.confirm('确认要删除选中信息吗？', {icon: 3,skin: 'layui-layer-lan',anim: 4,title:'删除'}, function(index) {
                layer.close(index);
                var loading = layer.load(1, {shade: [0.1, '#fff']});
                $.post("<?php echo url('tagdel'); ?>", {id: id}, function (data) {
                    layer.close(loading);
                    if (data.code === 1) {
                        layer.msg(data.msg);
                        tableIn.reload();
