...
class Trill extends Common
{
    ...
    public function accountlist()
    {
        if(request() -> isPost()){
            ...
            $field     = 'id,username,password,statu,addtime,tagname,codes';
            $order     = 'id';
            $query = [];
            $tagname = MODULENAME;
    ...
    public function accountadd()
    {
        ...
        if(request() -> isPost()){
            ...
            $count = db('user_account')->where($data)->find();
            if($count){
                return json(['code'=>0,'msg'=>'账号已经存在']);
            }
            $data['addtime'] = time();
            model('Common')
                ->leveltable('user_account_ping','user_account',200000);
            $arrjson = model('Common')
                -> add($data,'user_account');
            return json($arrjson);
        }else{
            return $this->fetch();
        }
    ...
    public function accountgroupadd()
    {
        if(request() -> isPost()){
            ...
            $arrs = $data;
            unset($arrs['name']);
            $count = db('user_accountgroup')->where($data)->find();
            if($count){
            ...
            $data['addtime'] = time();
            model('Common')
                ->leveltable('user_accountgroup_ping','user_accountgroup',200000);
            $arrjson = model('Common')
                -> add($data,'user_accountgroup');
            return json($arrjson);
        }
    }
    ...
    public function tasklist()
    {
        if(request() -> isPost()){
            $limit =input('limit')?input('limit'):10;
            $page = input('page')?input('page'):1;
            $field     = 'id,username,password,statu,addtime,phone,codes';
            $order     = 'id';
            $query = [];
            $tagname = MODULENAME;
            $starttime = input('post.starttime');
    ...
    public function index()
    {
        ...
        /*$limit =input('limit')?input('limit'):10;
        $page = input('page')?input('page'):100;
        $field     = '*';
        $order     = 'id';
        $query = [];
        $tagname = MODULENAME;
        ...
        $data = model('Common')->lists('userrecord',$limit,$page,$query,$field,$order);
     ...
     /*   dump($data);
        ...
        /* dump($data);exit;*/
        $wheres = [];
        $wheres['orderid'] = array('neq','');
        $stime=microtime(true);
       $start = db('userrecord')->field('id')->where($wheres)->order('id desc')->limit(1000,1)->find();

        //$start = Db::query("SELECT id FROM ad_userrecord WHERE orderid != '' ORDER BY id desc  LIMIT 1000,1");
        $etime=microtime(true);
        $total=$etime-$stime;
        dump($start);
        dump($total);exit;
        $end = Db::query("SELECT id FROM ad_userrecord WHERE orderid != '' ORDER BY id desc  LIMIT 990,1");
        $ids  = $start[0]['id'].','.$end[0]['id'];
        $map['id'] = ['between',$ids];
        $b = Db::name('userrecord')
            ->where($map)
            ->where($wheres)
            ->order('id','desc')
            ->limit(10)
    ...
    public function indexs()
    {
        // 1.创建员工信息数组
        $data = [];
        $data[] =['uid'=>'1','content'=>'ghgthgt','addtime'=>time()];
        $data[] =['uid'=>'2','content'=>'ghgthgtoioi','addtime'=>time()];
        $data[] =['uid'=>'3','content'=>'ghgthgtoiuo','addtime'=>time()];
        /*dump($data);exit;*/

        // 2.将数据集插入到表中
        $result = db('trill_comment')  //指定tp5_staff表
        ->insertAll($data);    // 批量添加
        $sql =  Db::getLastSql();
        // 3.查看返回结果  int(3)
        dump($result);
    ...
    public function index1()
    {
        ...
        $list = db('tag')
            ...
            ->select();
        //将二维数组转化成一维
        $list = array_column($list,'tagname');
        $list = implode(",",$list);
        dump( $this->tag);
